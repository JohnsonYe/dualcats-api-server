...
if (process.env.SSL_ENABLE != "true") {
    http.createServer(app).listen(process.env.DEFAULT_PORT || 8084, () => {
        console.log('Server is running on %s mode, listening at port %s', process.env.NODE_ENV , process.env.DEFAULT_PORT || 8084);
    });
} else {
